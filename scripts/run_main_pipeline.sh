...
for folder in "${folder_input_names[@]}"; do
    if [ -d "$INPUT_DATA/$folder" ]; then
        ...
        poetry run python -m data_validate.main --l=pt_BR --i=$INPUT_DATA/$folder/ --o=$OUTPUT_DATA/$folder/ --no-time --sector="Setor A" --protocol="Protocolo B" --user="Usuário C"
        
        if [ "$folder" == "data_errors_01" ]; then
            echo "Processando a pasta '$folder' com a flag --no-warning-titles-length..."
            poetry run python -m data_validate.main --l=pt_BR --i=$INPUT_DATA/$folder/ --o=$OUTPUT_DATA/$folder/no_titles_length/ --no-warning-titles-length --no-time --sector="Setor A" --protocol="Protocolo B" --user="Usuário C"
        fi

    ...
    try:
        ...
        if invalid_rows.any():
            ...
            for idx in invalid_indices:
                ...
                errors.append(f"{file_name}, linha {idx + 2}: A linha possui {non_null_counts[idx]} valores, mas a tabela possui apenas {valid_columns_count} {text_column}.")
...
def check_special_characters_cr_lf_columns_start_end(
    ...
    for column in existing_columns:
        ...
        patterns = {
            'end_cr': (text_series.str.endswith('\x0D'), "O texto da coluna '{column}' possui um caracter inválido (CR) no final do texto. Remova o último caractere do texto."),
            'end_lf': (text_series.str.endswith('\x0A'), "O texto da coluna '{column}' possui um caracter inválido (LF) no final do texto. Remova o último caractere do texto."),
            'start_cr': (text_series.str.startswith('\x0D'), "O texto da coluna '{column}' possui um caracter inválido (CR) no início do texto. Remova o primeiro caractere do texto."),
            'start_lf': (text_series.str.startswith('\x0A'), "O texto da coluna '{column}' possui um caracter inválido (LF) no início do texto. Remova o primeiro caractere do texto.")
        }
...
def check_special_characters_cr_lf_columns_anywhere(
    ...
    for column in existing_columns:
        ...
        def find_cr_lf_positions(text: str) -> List[Tuple[int, str]]:
            ...
            return [(match.start() + 1, "CR" if match.group() == '\x0D' else "LF")
        ...
        for idx, positions in cr_lf_positions.items():
            for pos, char_type in positions:
                warnings.append(f"{file_name}, linha {idx + 2}: O texto da coluna '{column}' possui um caracter inválido ({char_type}) na posição {pos}. Remova o caractere do texto.")
...
def check_text_length(
    ...
    if non_null_mask.any():
        ...
        for idx in text_lengths[exceeds_limit].index:
            ...
            errors.append(f'{file_name}, linha {idx + 2}: O texto da coluna "{column}" excede o limite de {max_length} caracteres (encontrado: {actual_length}).')

...
def get_min_max_legend(name_file, df_qml_legend):
    ...
    if (MIN_VALUE is None or MAX_VALUE is None) or (pd.isna(MIN_VALUE) or pd.isna(MAX_VALUE)):
        errors.append(
            f"{name_file}: Verificação de valores foi abortada porque os valores do arquivo QML '{SP_LEGEND_COLUMNS.NAME_SP}' não foram encontrados.")
        return errors, MIN_VALUE, MAX_VALUE
...
def check_tuple_sequence(value_list):
    ...
    for i in range(len(value_list) - 1):
        ...
        if VALOR_DIFF != 100 or current_tuple[1] >= next_tuple[0]:
            errors.append(
                f"Existe uma descontinuidade nos valores das fatias da legenda: {current_tuple[1]} e {next_tuple[0]}.")
    return errors
...
def check_overlapping(file_name, df_qml_legend):
    ...
    if len(lower_values) != len(upper_values):
        errors.append(
            f"{file_name}: Valores insuficientes para delimitar as fatias. Existem elementos 'lower' ou 'upper' com valores inválidos ou ausentes.")
        return not errors, errors
    ...
    for _, row in df_qml_legend.iterrows():
        if row[SP_LEGEND_COLUMNS.LOWER] > row[SP_LEGEND_COLUMNS.UPPER]:
            ...
            errors.append(
                f"{file_name}: Existe uma sobreposição nos valores das fatias da legenda. O elemento 'lower' ({lower_i}) é maior que o elemento 'upper' ({upper_i}).")
...
def extract_ids_from_list(list_values, values_scenario=[]):
    ...
    cleaned_columns = [column for column in list_values if
                       pattern_id_year.match(column) or pattern_id_year_scenario.match(column)]
...
def read_legend_qml_file(qml_file_path):
    ...
    try:
        ...
        if root.find(SP_LEGEND_COLUMNS.KEY_RENDERER_V2) is None:
            errors.append(
                f"Erro ao processar o arquivo {base_name}: Não foi encontrado o elemento '{SP_LEGEND_COLUMNS.KEY_RENDERER_V2}'.")
        ...
        if root.find(SP_LEGEND_COLUMNS.KEY_RENDERER_V2).find(SP_LEGEND_COLUMNS.KEY_RANGES) is None:
            errors.append(
                f"Erro ao processar o arquivo {base_name}: Não foi encontrado o elemento '{SP_LEGEND_COLUMNS.KEY_RANGES}' dentro do elemento '{SP_LEGEND_COLUMNS.KEY_RENDERER_V2}'.")
            return pd.DataFrame(), errors
    ...
    if file_extension == ".csv":
        try:
            ...
            if not any(enc in file_encoding.lower() for enc in ['ascii', 'utf-8']):
                errors.append(f"{filename}: O arquivo está no formato {file_encoding}, deveria ser UTF-8.")
            try:
                df = pd.read_csv(path, delimiter=DEFAULT_SEPARATOR, encoding=ENCODING, low_memory=False, dtype=str,
                                 header=[0, 1])
            except Exception:
                return pd.DataFrame(), errors
    ...
    try:
        ...
        if is_csv and is_xlsx:
            return True, is_csv, is_xlsx, [
                f"{file_name}: Existe um arquivo .csv e um arquivo .xlsx com o mesmo nome. Será considerado o arquivo .csv."]
            ...
            for column in df_sp.columns:
                if mask[column].any():
                    ...
                    for row in rows_with_error:
                        errors.append(
                            f"{name_file}, linha {row + 2}: A coluna '{column}' não pode conter o caracter '|'.")

...
from ..locale.language_enum import LanguageEnum
...
class LanguageManager:
    ...
    def __init__(self, locale_dir=None):
        ...
        Args:
            locale_dir (str): Path to the directory containing locale files.
        """
        self.locale_dir = locale_dir or os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', 'static/locales'))
    ...
    def _congifure_language(self):
        store_locale_path = Path(__file__).resolve().parents[3] / '.config' / 'store.locale'
    ...
    def _load_translations(self, lang_code):
        ...
        filepath = os.path.join(self.locale_dir, lang_code, 'messages.json')
        try:

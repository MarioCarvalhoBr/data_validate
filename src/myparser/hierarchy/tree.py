    ...
    if erros_niveis:
        for pai, filho in erros_niveis:
            if (filho is not None) and (pai is not None): 
                ...
                errors.append(f"{SP_COMPOSITION_COLUMNS.NAME_SP}, linha {index_linha}: O indicador {pai} (nível {nivel_pai}) não pode ser pai do indicador {filho} (nível {nivel_filho}). Atualize os níveis no arquivo de {SP_DESCRIPTION_COLUMNS.NAME_SP}.")
    return not errors, errors, warnings

...
class TestValidateNumericValue:
    ...
    def test_validate_numeric_value_valid_cases(
        ...
        is_valid, error_msg, has_excessive_decimals = ValueProcessing.check_numeric_value(value, row_index, column, filename)
    ...
    def test_validate_numeric_value_invalid_cases(self, value: Any, row_index: int, column: str, filename: str) -> None:
    ...
    def test_validate_numeric_value_di_case(self) -> None:
        ...
        is_valid, error_msg, has_excessive_decimals = ValueProcessing.check_numeric_value("DI", 5, "codigo", "dados.xlsx")
    ...
    def test_validate_numeric_value_decimal_processing_error(self) -> None:
        ...
        is_valid, error_msg, has_excessive_decimals = ValueProcessing.check_numeric_value("invalid_text", 0, "test_col", "test.xlsx")
...
class TestProcessColumnValidation:
    ...
    def test_process_column_validation_valid_column(self, sample_dataframe: pd.DataFrame) -> None:
        """Test process_column_validation with valid column."""
        errors, excessive_decimal_rows = ValueProcessing.process_column_validation(sample_dataframe, "valid_col", "test.xlsx")

        assert len(errors) == 0
        assert len(excessive_decimal_rows) == 0

    def test_process_column_validation_invalid_column(self, sample_dataframe: pd.DataFrame) -> None:
        """Test process_column_validation with invalid values."""
        errors, excessive_decimal_rows = ValueProcessing.process_column_validation(sample_dataframe, "invalid_col", "test.xlsx")
    ...
    def test_process_column_validation_excessive_decimals(self, sample_dataframe: pd.DataFrame) -> None:
        """Test process_column_validation with excessive decimal places."""
        errors, excessive_decimal_rows = ValueProcessing.process_column_validation(sample_dataframe, "excessive_decimals", "test.xlsx")
    ...
    def test_process_column_validation_single_invalid_value(self) -> None:
        ...
        errors, excessive_decimal_rows = ValueProcessing.process_column_validation(df, "col", "test.xlsx")
...
class TestGenerateDecimalWarning:
    ...
    def test_generate_decimal_warning_no_excessive_decimals(self) -> None:
        ...
        warning = ValueProcessing.generate_decimal_warning(set(), 0, "test.xlsx")
    ...
    def test_generate_decimal_warning_single_value(self) -> None:
        ...
        warning = ValueProcessing.generate_decimal_warning({5}, 1, "test.xlsx")
    ...
    def test_generate_decimal_warning_multiple_values(self) -> None:
        ...
        warning = ValueProcessing.generate_decimal_warning({2, 5, 8}, 3, "test.xlsx")
    ...
    def test_generate_decimal_warning_count_mismatch(self) -> None:
        ...
        warning = ValueProcessing.generate_decimal_warning({2, 5}, 5, "dados.xlsx")
    ...
    def test_generate_decimal_warning_grammar(self, rows: Set[int], count: int, expected_existem: str, expected_valores: str) -> None:
        """Test generate_decimal_warning grammar for singular/plural forms."""
        warning = ValueProcessing.generate_decimal_warning(rows, count, "test.xlsx")
...
class TestValidateDataValuesInColumns:
    ...
    def test_validate_data_values_in_columns_valid_columns(self, sample_dataframe: pd.DataFrame) -> None:
        """Test validate_data_values_in_columns with valid columns only."""
        errors, warnings = ValueProcessing.validate_data_values_in_columns(sample_dataframe, ["col1"], "test.xlsx")

        assert len(errors) == 0
        assert len(warnings) == 0

    def test_validate_data_values_in_columns_excessive_decimals(self, sample_dataframe: pd.DataFrame) -> None:
        """Test validate_data_values_in_columns with excessive decimal places."""
        errors, warnings = ValueProcessing.validate_data_values_in_columns(sample_dataframe, ["col2"], "test.xlsx")

        assert len(errors) == 0
        assert len(warnings) == 1
        assert "Existem 3 valores com mais de 2 casas decimais" in warnings[0]

    def test_validate_data_values_in_columns_invalid_values(self, sample_dataframe: pd.DataFrame) -> None:
        """Test validate_data_values_in_columns with invalid values."""
        errors, warnings = ValueProcessing.validate_data_values_in_columns(sample_dataframe, ["col3"], "test.xlsx")

        assert len(errors) == 1
        assert "2 valores que não são número válido nem DI" in errors[0]
        assert len(warnings) == 0

    def test_validate_data_values_in_columns_all_invalid(self, sample_dataframe: pd.DataFrame) -> None:
        """Test validate_data_values_in_columns with all invalid values."""
        errors, warnings = ValueProcessing.validate_data_values_in_columns(sample_dataframe, ["col4"], "test.xlsx")

        assert len(errors) == 1
        assert "4 valores que não são número válido nem DI" in errors[0]
        assert len(warnings) == 0

    def test_validate_data_values_in_columns_multiple_columns(self, sample_dataframe: pd.DataFrame) -> None:
        """Test validate_data_values_in_columns with multiple columns."""
        errors, warnings = ValueProcessing.validate_data_values_in_columns(sample_dataframe, ["col1", "col2", "col3"], "test.xlsx")
    ...
    def test_validate_data_values_in_columns_empty_dataframe(self) -> None:
        ...
        errors, warnings = ValueProcessing.validate_data_values_in_columns(df, ["col"], "test.xlsx")

        assert len(errors) == 0
        assert len(warnings) == 0

    def test_validate_data_values_in_columns_empty_columns_list(self, sample_dataframe: pd.DataFrame) -> None:
        """Test validate_data_values_in_columns with empty columns list."""
        errors, warnings = ValueProcessing.validate_data_values_in_columns(sample_dataframe, [], "test.xlsx")

        assert len(errors) == 0
        assert len(warnings) == 0

    def test_validate_data_values_in_columns_nonexistent_column(self, sample_dataframe: pd.DataFrame) -> None:
        """Test validate_data_values_in_columns with nonexistent column."""
        with pytest.raises(KeyError):
            ValueProcessing.validate_data_values_in_columns(sample_dataframe, ["nonexistent_col"], "test.xlsx")
    ...
    def test_validate_data_values_in_columns_comprehensive_scenario(self) -> None:
        ...
        errors, warnings = ValueProcessing.validate_data_values_in_columns(
            df,
        ...
        assert len(warnings) == 1
        assert "Existem 6 valores com mais de 2 casas decimais" in warnings[0]  # Updated count
...
class TestEdgeCases:
    ...
    def test_very_large_numbers(self) -> None:
        ...
        large_number = 1e20
        is_valid, error_msg, has_excessive_decimals = ValueProcessing.check_numeric_value(large_number, 0, "test_col", "test.xlsx")
    ...
    def test_very_small_numbers(self) -> None:
        ...
        small_number = 1e-10
        is_valid, error_msg, has_excessive_decimals = ValueProcessing.check_numeric_value(small_number, 0, "test_col", "test.xlsx")
    ...
    def test_zero_variations(self) -> None:
        ...
        for zero_val in zero_values:
            is_valid, error_msg, has_excessive_decimals = ValueProcessing.check_numeric_value(zero_val, 0, "test_col", "test.xlsx")
    ...
    def test_negative_numbers_with_decimals(self) -> None:
        ...
        is_valid, error_msg, has_excessive_decimals = ValueProcessing.check_numeric_value(-10.555, 0, "test_col", "test.xlsx")
    ...
    def test_scientific_notation(self) -> None:
        ...
        is_valid, error_msg, has_excessive_decimals = ValueProcessing.check_numeric_value("1.5e2", 0, "test_col", "test.xlsx")
    ...
    def test_special_float_values(self) -> None:
        ...
        is_valid, error_msg, has_excessive_decimals = ValueProcessing.check_numeric_value(float("inf"), 0, "test_col", "test.xlsx")

...
class TestCheckPunctuation:
    ...
    def sample_dataframe(self) -> pd.DataFrame:
        ...
        return pd.DataFrame({
            ...
            'descricao': ['Descrição completa.', 'Descrição sem ponto', 'Descrição com exclamação!'],
    ...
    def test_check_punctuation_handles_none_values(self) -> None:
        ...
        # Arrange
        df = pd.DataFrame({
            'test_col': [None, '', 'Valid text.', 'Invalid text,']
        })
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ('Texto correto.', True, False),  # Fixed: dot is punctuation, so should warn for no_punct
    ...
    def test_check_punctuation_individual_text_cases(
        self,
        text_value: str,
        should_warn_no_punct: bool,
        should_warn_need_dot: bool
    ) -> None:
...
class TestCheckSpecialCharactersCrLf:
    ...
    def test_find_cr_lf_positions_parametrized(
        ...
        expected_positions: List[Tuple[int, str]]
    ) -> None:
    ...
    def test_check_special_characters_empty_columns_list(
        ...
        # Act
        is_valid_start_end, warnings_start_end = check_special_characters_cr_lf_columns_start_end(
            dataframe_with_cr_lf_characters, "test.xlsx", []
        )

        is_valid_anywhere, warnings_anywhere = check_special_characters_cr_lf_columns_anywhere(
            dataframe_with_cr_lf_characters, "test.xlsx", []
        )
...
class TestCheckTextLength:
    ...
    def dataframe_with_various_text_lengths(self) -> pd.DataFrame:
        ...
        return pd.DataFrame({
            'short_text': ['A', 'AB', 'ABC'],
            'medium_text': ['Medium length text here', 'Another medium text', 'Third medium'],
            'long_text': [
            ...
            'mixed_length': ['Short', 'Medium length text', 'Very long text that exceeds limits']
        })
...
class TestDataValidationIntegration:
    ...
    def complex_test_dataframe(self) -> pd.DataFrame:
        ...
        return pd.DataFrame({
            ...
            'texto_longo': ['Short', 'This is a very long text that exceeds limits', 'OK'],
